...
export class UtilMegamanStatus {
  public static create(hpMemoryNum: number = 45) : MegamanStatus {
    return {
      hpMemoryNum,
      statuses: {
        hp: new StatusHp(100),
  ...
  private static abilityKeyToStatusKey = {
    'hp-plus': 'hp',
    'attack-plus': 'attack',
    'rapid-plus': 'rapid',
    'charge-plus': 'charge',
  ...
  private static applyAbility(megamanStatus: MegamanStatus, ability: AbilityBase) : MegamanStatus {
    switch (ability.key) {
      case 'hp-magnify':
        megamanStatus.statuses.hp.applyMagnify(ability.value as number);
        break;

      case 'attack-magnify':
        megamanStatus.statuses.attack.applyMagnify(ability.value as number);
        break;

      case 'damage-bug-fix':
        megamanStatus.statuses.damageHpBug.apply(ability.value as boolean);
        megamanStatus.statuses.damagePanicBug.apply(ability.value as boolean);
        break;

      case 'status-bug-change':
        megamanStatus.statuses.statusBug.apply(ability.value as number);
        break;

      case 'battle-hp-bug-change':
        megamanStatus.statuses.battleHpBug.apply(ability.value as number);
        break;

      case 'buster-bug-change':
        megamanStatus.statuses.busterBug.apply(ability.value as number);
        break;

      default:
        if (megamanStatus.statuses[this.abilityKeyToStatusKey[ability.key]] instanceof StatusBaseNumber) {
          megamanStatus.statuses[this.abilityKeyToStatusKey[ability.key]].applyPlus(ability.value);
          break;
        }
        megamanStatus.statuses[this.abilityKeyToStatusKey[ability.key]].apply(ability.value);
        break;
    }

    return megamanStatus;
  }

  private static applyHpMemory(megamanStatus: MegamanStatus) : MegamanStatus {
    megamanStatus.statuses.hp.applyPlus(megamanStatus.hpMemoryNum * 20);
    return megamanStatus;
  }

  private static applyAbilities(megamanStatus: MegamanStatus) : MegamanStatus {
    let result = { ...megamanStatus };
    // サポートバグが存在するかチェック
    const hasSupportBug = megamanStatus.abilities.some((ability) => ability.key === 'support-bug');

    if (hasSupportBug) {
      result.abilities = megamanStatus.abilities.filter((ability) => !(ability.key === 'rush-support' || ability.key === 'beat-support' || ability.key === 'tango-support'));
    }

    result.abilities.forEach((ability) => {
      result = this.applyAbility(megamanStatus, ability);
    });

    return megamanStatus;
  }

  public static apply(megamanStatus: MegamanStatus) : MegamanStatus {
    let result = { ...megamanStatus };
    result = this.applyHpMemory(result);
    result = this.applyAbilities(result);
    return result;
  }
}
